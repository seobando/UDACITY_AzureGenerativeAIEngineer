...
def retrieve(query: str, index_name: str, top_k: int = 3) -> str:
    ...
    # These should be set in your Azure AI Studio connection or environment
    ...
    if not search_endpoint or not search_api_key:
        raise ValueError(
            "Azure AI Search credentials not found. Please set AZURE_SEARCH_ENDPOINT "
            "(or AZURE_SEARCH_SERVICE_NAME) and AZURE_SEARCH_API_KEY environment variables."
        )
    ...
    # The search_type parameter can be set to "semantic", "vector", or defaults to hybrid
    ...
    for result in results:
        ...
        if not content:
            ...
            content = " ".join([str(v) for k, v in result.items() if isinstance(v, str) and k != "id"])
        ...
        doc_text = content
        if metadata:
            doc_text = f"[Score: {metadata.get('score', 'N/A')}] {content}"
